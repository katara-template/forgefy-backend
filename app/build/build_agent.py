...
def run_build_agent(
    ...
    return _loop(client, model, _BUILD_SYSTEM, workspace, user_msg)
...
def run_update_agent(
    ...
    return _loop(client, model, _UPDATE_SYSTEM, workspace, user_msg)
    ...
    for _ in range(_MAX_ITERATIONS):
        ...
        for block in response.content:
            if block.type == "text":
                ...
                logger.debug("Tool %s → %s", block.name, result[:80])
                tool_results.append(
